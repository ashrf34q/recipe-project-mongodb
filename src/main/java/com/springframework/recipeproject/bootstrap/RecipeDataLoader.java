...
public class RecipeDataLoader implements ApplicationListener<ContextRefreshedEvent>{
	...
	public RecipeDataLoader(RecipeRepository recipeRepository, UnitOfMeasureRepository unitOfMeasureRepo,
	...
	private List<Recipe> getRecipes(){
		...
		Optional<UnitOfMeasure> eachUOM = unitOfMeasureRepo.findByDescription("Each");
		checkUOM(eachUOM);
		
		Optional<UnitOfMeasure> teaspoonUOM = unitOfMeasureRepo.findByDescription("Teaspoon");
		checkUOM(teaspoonUOM);
		
		Optional<UnitOfMeasure> tablespoonUOM = unitOfMeasureRepo.findByDescription("Tablespoon");
		checkUOM(eachUOM);
		
		Optional<UnitOfMeasure> cupUOM = unitOfMeasureRepo.findByDescription("Cup");
		checkUOM(eachUOM);
		
		Optional<UnitOfMeasure> pintUOM = unitOfMeasureRepo.findByDescription("Pint");
		checkUOM(eachUOM);
		
		Optional<UnitOfMeasure> dashUOM = unitOfMeasureRepo.findByDescription("Dash");
		checkUOM(eachUOM);
		
		Optional<UnitOfMeasure> pinchUOM = unitOfMeasureRepo.findByDescription("Pinch");
		checkUOM(eachUOM);
	
		//get unit of measures
			UnitOfMeasure eachUom = eachUOM.get();
	        UnitOfMeasure tableSpoonUom = tablespoonUOM.get();
	        UnitOfMeasure teaspoonUom = teaspoonUOM.get();
	        UnitOfMeasure dashUom = dashUOM.get();
	        UnitOfMeasure pintUom = pintUOM.get();
	        UnitOfMeasure cupsUom = cupUOM.get();
	        UnitOfMeasure pinchUom = pinchUOM.get();
	        
	    //do the same thing with categories
	        Optional<Category> mexicanCategory = categoryRepository.findByDescription("Mexican");
	        checkCategory(mexicanCategory);
	        
	        Optional<Category> americanCategory = categoryRepository.findByDescription("American");
	        checkCategory(americanCategory);
	        
	     //get categories
	        Category american = americanCategory.get();
	        Category mexican = mexicanCategory.get();
	        
	        //Set the guacamole recipe
	        ...
	        guacRecipe.getIngredients().add(new Ingredients("Kosher salt", new BigDecimal(".5"), teaspoonUom, guacRecipe));
	        ...
	        tacosRecipe.getIngredients().add(new Ingredients("Dried Oregano", new BigDecimal(1), teaspoonUom, tacosRecipe));
	        tacosRecipe.getIngredients().add(new Ingredients("Dried Cumin", new BigDecimal(1), teaspoonUom, tacosRecipe));
	        tacosRecipe.getIngredients().add(new Ingredients("Sugar", new BigDecimal(1), teaspoonUom, tacosRecipe));
	        tacosRecipe.getIngredients().add(new Ingredients("Salt", new BigDecimal(".5"), teaspoonUom, tacosRecipe));
	...
	//This method checks if the unit of measure is present in the repository. If not, throws an exception 
	private void checkUOM(Optional<UnitOfMeasure> UOM) {
		if(!UOM.isPresent()) {
	...
	private void checkCategory(Optional<Category> category) {
